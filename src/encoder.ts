...
export function deserialize(str: string): IState {
    ...
    state.kbd = Object.fromEntries(str.slice(9, 35).split('').map(x => +x).map((x, i) => {
        return [String.fromCharCode(i + 97), x]
    })) as TKeyboard

    const wordsLen = state.row * 5 + state.col
    const words = [...str.slice(35, 35 + wordsLen).split(''), ...new Array(30 - wordsLen).fill(null)]
    const wrd: string[][] = []
    while (words.length) wrd.push(words.splice(0, 5))
    state.wrd = wrd as TGuesses

    const feedbacksLen = state.row * 5
    const feedbacks = [...str.slice(35 + wordsLen, 35 + wordsLen + feedbacksLen).split('').map(x => +x), ...new Array(30 - feedbacksLen).fill(EFeedback.empty)]
    const fbk: number[][] = []
...
export function bin5ToHex(str: string): string {
    return str.match(/[01]{1,4}/g)?.map(b => {

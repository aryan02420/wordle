...
import { removeAlphabet, addAplhabet, evaluateGuess, updateKeyboard, submitGuess } from '../src/utils'
...
describe('evaluateGuess', () => {
    ...
    test('correct evaluation #1', () => {
        expect(evaluateGuess(['s', 'p', 'e', 'e', 'd'], ['a', 'b', 'i', 'd', 'e']))
            .toEqual([EFeedback.absent, EFeedback.absent, EFeedback.present, EFeedback.absent, EFeedback.present])
    })

    test('correct evaluation #2', () => {
        expect(evaluateGuess(['s', 'p', 'e', 'e', 'd'], ['e', 'r', 'a', 's', 'e']))
            .toEqual([EFeedback.present, EFeedback.absent, EFeedback.present, EFeedback.present, EFeedback.absent])
    })

    test('correct evaluation #3', () => {
        expect(evaluateGuess(['s', 'p', 'e', 'e', 'd'], ['s', 't', 'e', 'a', 'l']))
            .toEqual([EFeedback.correct, EFeedback.absent, EFeedback.correct, EFeedback.absent, EFeedback.absent])
    })

    test('correct evaluation #4', () => {
        expect(evaluateGuess(['s', 'p', 'e', 'e', 'd'], ['c', 'r', 'e', 'p', 'e']))
            .toEqual([EFeedback.absent, EFeedback.present, EFeedback.correct, EFeedback.present, EFeedback.absent])
    })
...
describe('updateKeyboard', () => {
    ...
    test('keyboard showes highest information', () => {
        expect(updateKeyboard(
            { a: EFeedback.absent, b: EFeedback.correct, c: EFeedback.present, d: EFeedback.correct, e: EFeedback.present } as TKeyboard,
            ['a', 'b', 'c', 'd', 'e'],
            [EFeedback.present, EFeedback.present, EFeedback.correct, EFeedback.correct, EFeedback.present]
        )).toEqual(
            { a: EFeedback.present, b: EFeedback.correct, c: EFeedback.correct, d: EFeedback.correct, e: EFeedback.present },
        )
    })
